...
mylinestyles = {
 'SLDNet (L:0.001)': 'solid',
 'SLDNet (L:4)': 'solid',
 'DiMMAD (min-med)': (0, (1, 1)),
 'DiMMAD (med-med)': (0, (5, 5)),
 'iForest': 'dotted',
 'LOF': 'dashdot',
 'OC-SVM': 'dashed',
...
def _fit_requires_y(model):
    try:
        sig = inspect.signature(model.fit)
    except (TypeError, ValueError):
        return False

    y_param = sig.parameters.get("y")
...
def run_experiment(
    ...
    for i in tqdm(range(N_RUNS), desc="Running Experiments"):
        ...
        for model_name, model_template in models_to_test.items():
            ...
            if _fit_requires_y(model) or "DiMMAD" in model_name or "MCSVDD" in model_name:
...
def analysis_with_errors(
    ...
    if metric == "purity":
        plt.ylabel("Mean Purity", fontsize=20)
    elif metric == "diversity":
        plt.ylabel("New Classes Discovered", fontsize=24)

    plt.xlabel("Follow-up Budget (No. of sources)", fontsize=24)
    ...
    if show_legend:
        ...
        # Standard legend bottom right
        loc = "upper right" if metric == "purity" else "lower right"
        legend = plt.legend(
            loc=loc,
        ...
        for text in legend.get_texts():
            if "sbgm" in text.get_text().lower():
